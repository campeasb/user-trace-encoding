...
class UsernameTransformer(nn.Module):
    ...
    d'actions avec un contexte multi-tokens.

    - En entraînement: prend (username, action_sequence, browser, duration_bucket, speed_bucket).
    - En inférence: prend (action_sequence, browser, duration_bucket, speed_bucket) et prédit l'utilisateur.
    """
    ...
    def __init__(
        ...
        # Encodage positionnel (augmenté du nombre de tokens de contexte)
    ...
    def forward(self, action_sequence, browser=None, duration_bucket=None, speed_bucket=None, username=None, training=True):
        """
        Passe avant du transformer avec contexte multi-tokens.

        Arguments:
            action_sequence: tenseur (batch_size, seq_len) d'IDs de tokens d'actions.
            browser: tenseur (batch_size,) d'IDs de navigateurs.
            duration_bucket: tenseur (batch_size,) des IDs de classes de durée.
            speed_bucket: tenseur (batch_size,) des IDs de classes de vitesse.
        ...
        # Créer les embeddings des tokens de contexte
        context_embs = []
        context_masks = []
        ...
        # Concaténer d'abord les tokens de contexte, puis les tokens d'action
        if context_embs:
            x = torch.cat(context_embs + [action_emb], dim=1)  # (batch_size, n_contexts + seq_len, d_model)
        else:
            x = action_emb
        
        # Créer le masque de padding (étendu pour inclure les tokens de contexte)
        ...
        # Moyenne globale (pooling) sur la séquence en excluant les tokens de contexte
        # Utiliser le masque pour ignorer les tokens de padding et exclure le contexte du pooling
        ...
        # Ne moyenner que sur les tokens d'action (ignorer les tokens de contexte en tête)
    ...
    def predict_username(self, action_sequence, browser=None, duration_bucket=None, speed_bucket=None):
        """
        Prédire l'utilisateur à partir de la séquence d'actions et des tokens de contexte (mode inférence).

        Arguments:
            action_sequence: tenseur (batch_size, seq_len) ou (seq_len,)
            browser: tenseur (batch_size,) ou scalaire d'IDs de navigateurs
            duration_bucket: tenseur (batch_size,) ou scalaire des IDs de classes de durée
            speed_bucket: tenseur (batch_size,) ou scalaire des IDs de classes de vitesse
...
def train_model(model, train_data, val_data, learning_rate=1e-5, epochs=50, batch_size=8, max_seq_len=100, device='cpu'):
    """
    Entraîner le modèle UsernameTransformer avec contexte multi-tokens.

    Arguments:
        model: instance de UsernameTransformer.
        train_data: tuple (action_sequences, usernames, browsers, duration_buckets, speed_buckets) ou variantes.
        val_data: tuple (action_sequences, usernames, browsers, duration_buckets, speed_buckets) ou variantes.
